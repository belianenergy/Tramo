...
// Cargar o prompt v4 en español con Datadis e simulación baterías
const prompt = fs.readFileSync('STITCH_PROMPT_ES_V4.md', 'utf8');
...
async function main() {
  console.log('1. Creando proxecto Stitch v4...');
  const project = await callStitch('create_project', { title: 'EnergyOS Dashboard v4 - Datadis + Simulación Baterías' });
  ...
  console.log('\n2. Xerando pantalla do dashboard v4 en español (estilo amigable)...');
  ...
  fs.writeFileSync('stitch-v4-response.json', JSON.stringify(screen, null, 2));
  console.log('Resposta gardada en stitch-v4-response.json');
  ...
  if (downloadUrl) {
    ...
    execSync(`curl -L -o stitch-dashboard-v4.html "${downloadUrl}"`, { stdio: 'inherit' });
    console.log('HTML descargado como stitch-dashboard-v4.html');
    
    // Verificar que está en español e modo claro
    const html = fs.readFileSync('stitch-dashboard-v4.html', 'utf8');
    const isSpanish = html.includes('lang="es"') || html.includes('lang="gl"') || html.includes('Panel') || html.includes('Comunidades');
    const isLightMode = html.includes('class="light"') || html.includes('#FAFAFA') || html.includes('#fafafa');
    const hasDatadis = html.includes('Datadis') || html.includes('datadis') || html.includes('autorización') || html.includes('Autorización');
    const hasBatterySim = html.includes('batería') || html.includes('Batería') || html.includes('Simulación') || html.includes('simulación');
    console.log('En español:', isSpanish);
    console.log('Modo claro:', isLightMode);
    console.log('Ten Datadis:', hasDatadis);
    console.log('Ten simulación baterías:', hasBatterySim);
    ...
    const screenshotScript = `
      ...
      const htmlPath = path.resolve('stitch-dashboard-v4.html');
      ...
      (async () => {
        ...
        await page.screenshot({ path: 'stitch-dashboard-v4-preview.png', fullPage: true });
        console.log('Screenshot gardado como stitch-dashboard-v4-preview.png');
        await browser.close();
      })();
    `;
    fs.writeFileSync('screenshot-v4.mjs', screenshotScript);
    execSync('node screenshot-v4.mjs', { stdio: 'inherit' });
  } else {
